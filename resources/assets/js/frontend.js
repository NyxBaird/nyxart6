...
function minimizeHeaderLinks() {
    ...
    if (headerOverflown() && !roomForLinks()) {
        if (!$links.hasClass('minimized'))
            $links.addClass('minimized');
        ...
        if ($icon.siblings('.linksNotch').css('border-width').split(" ").length === 3)
            $icon.css({'background-color': lighten(pageColor, 20)});
        else
            $icon.css({'background-color': pageColor});
...
function lighten(color, percent) {
    ...
    if (color.length < 3)
        console.log("Looks like you're trying to lighten a non-color!");
...
function roomForLinks() {
    ...
    if ($('#header')[0].offsetWidth - offset > liLength)
        return true;
